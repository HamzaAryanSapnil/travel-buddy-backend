import { z } from "zod";
import { expenseCategoryEnum, expenseSplitTypeEnum } from "./expense.constant";
...
const participantSchema = z.object({
  ...
  percentage: z.number().min(0, { message: "Percentage must be >= 0." }).max(100, { message: "Percentage must be <= 100." }).optional(),
});
...
const createExpenseSchema = z.object({
  body: z
    .object({
      ...
      category: z.enum(expenseCategoryEnum as unknown as [string, ...string[]], {
      ...
      splitType: z.enum(expenseSplitTypeEnum as unknown as [string, ...string[]], {
    ...
    .refine(
      (data) => {
        if (data.splitType === "CUSTOM") {
          return data.participants?.every((p) => p.amount !== undefined && p.percentage === undefined);
        }
        if (data.splitType === "PERCENTAGE") {
          return data.participants?.every((p) => p.percentage !== undefined && p.amount === undefined);
        }
        return true;
      },
      {
...
const updateExpenseSchema = z.object({
  ...
  body: z.object({
    ...
    currency: stringRequired("Currency must be a string.").max(10, { message: "Currency code cannot exceed 10 characters." }).optional(),
    category: z
      .enum(expenseCategoryEnum as unknown as [string, ...string[]], {
...
const getExpensesSchema = z.object({
  query: z.object({
    ...
    splitType: z
      .enum(expenseSplitTypeEnum as unknown as [string, ...string[]], {
