/**
 * UI Step Types for Planner Flow
 * Each step represents a stage in the travel planning conversation
 */
...
export const UI_STEPS: UIStepType[] = [
  "destination",
  "groupSize",
  "budget",
  "tripDuration",
 ...
 */
export const INITIAL_QUESTION = "What is your travel destination?";

/**
 * System prompt for AI travel planner
 */
export const SYSTEM_PROMPT = `You are a helpful travel planning assistant. Your role is to guide users through creating a personalized travel itinerary by asking relevant questions one at a time.
 ...
 */
export const getPromptForStep = (currentStep: UIStepType, conversationHistory: string[]): string => {
 ...
 */
export const getNextStep = (currentStep: UIStepType): UIStepType | null => {
  const currentIndex = UI_STEPS.indexOf(currentStep);
  if (currentIndex === -1 || currentIndex >= UI_STEPS.length - 1) {
    return null; // No next step
  }
  return UI_STEPS[currentIndex + 1];
};
